...
void ImGui_ImplOTCV_BuildBuffers(otcv::VertexBuffer* vb, otcv::Buffer*& ib) {
    ...
    int vb_size = draw_data->TotalVtxCount * sizeof(ImDrawVert);
    int ib_size = draw_data->TotalIdxCount * sizeof(ImDrawIdx);
    if (vb_size > vb->buffers[0]->builder._info.size) {
        vb->resize(0, vb_size);
    }
    if (ib_size > ib->builder._info.size) {
        otcv::BufferBuilder builder = ib->builder;
        ib->destroy();
        ib = builder.size(ib_size).build();
    }
    ...
    for (int i = 0; i < draw_data->CmdListsCount; ++i) {
        ...
        uint32_t vtx_block_size = cmd_list->VtxBuffer.Size * sizeof(ImDrawVert);
        uint32_t idx_block_size = cmd_list->IdxBuffer.Size * sizeof(ImDrawIdx);
        vb->buffers[0]->copy_host_mapped(cmd_list->VtxBuffer.Data, vtx_offset, vtx_block_size);
        ib->copy_host_mapped(cmd_list->IdxBuffer.Data, idx_offset, idx_block_size);
...
void ImGui_ImplOTCV_Commands(otcv::CommandBuffer* command_buffer, otcv::Image* target, ImGui_ImplOTCV_SynchronizationInfo* info) {
    ...
    
    {
        ...
        ImGui_ImplOTCV_Exec(command_buffer);
...
void ImGui_ImplOTCV_Exec(otcv::CommandBuffer* command_buffer, otcv::VertexBuffer* vb, otcv::Buffer* ib) {
    ImGui_ImplOTCV_Data* bd = ImGui_ImplOTCV_GetBackendData();
    ...
    command_buffer->cmd_bind_vertex_buffer(vb);
    command_buffer->cmd_bind_index_buffer(ib, sizeof(ImDrawIdx) == 2 ? VK_INDEX_TYPE_UINT16 : VK_INDEX_TYPE_UINT32);
...
void ImGui_ImplOTCV_RenderDrawData(otcv::Image* target, ImGui_ImplOTCV_SynchronizationInfo* info) {
    ...
    // ImGui_ImplOTCV_BuildDescriptorSet();
